...
import {
  ...
  updateSectionTitle as storeUpdateSectionTitle,
  updateLessonTitle as storeUpdateLessonTitle,
  updateLessonVideoPath as storeUpdateLessonVideoPath,
  batchReplaceSectionVideos as storeBatchReplaceSectionVideos,
  deleteLesson as storeDeleteLesson,
  deleteSection as storeDeleteSection,
} from "@/lib/store";

const videoExtensions = ["mp4", "mkv", "avi", "mov", "webm", "flv", "wmv", "m4v", "mpg", "mpeg", "ts", "m2ts"];
...
export function CourseEditPanel({
  ...
  const handleSectionTitleChange = useCallback(
    (sectionId: number, newTitle: string) => {
      ...
      storeUpdateSectionTitle(sectionId, newTitle)
        .then(() => toast.success(t("courseEdit.sectionTitleUpdated")))
        .catch(() => {
          toast.error(t("courseEdit.sectionTitleUpdateFailed"));
          onReorder().catch(() => {});
        });
    },
    [onReorder, t],
  ...
  const handleLessonTitleChange = useCallback(
    (sectionId: number, lessonId: number, newTitle: string) => {
      setLocalSections((prev) =>
        prev.map((s) =>
          s.id !== sectionId
            ? s
      ...
      storeUpdateLessonTitle(lessonId, newTitle)
        .then(() => toast.success(t("courseEdit.lessonTitleUpdated")))
        .catch(() => {
          toast.error(t("courseEdit.lessonTitleUpdateFailed"));
          onReorder().catch(() => {});
        });
    },
    [onReorder, t],
  ...
  const handleLessonVideoReplace = useCallback(
    async (sectionId: number, lessonId: number) => {
      const selected = await open({
        multiple: false,
        title: t("courseEdit.selectVideoFile"),
        filters: [{ name: t("courseEdit.videoFile"), extensions: videoExtensions }],
      });
      if (!selected || typeof selected !== "string") return;
      try {
        await storeUpdateLessonVideoPath(lessonId, selected);
        setLocalSections((prev) =>
          prev.map((s) =>
            s.id !== sectionId
              ? s
              : {
                  ...s,
                  lessons: s.lessons.map((l) =>
                    l.id === lessonId ? { ...l, videoPath: selected, lastPosition: 0 } : l,
                  ),
                },
          ),
        );
        toast.success(t("courseEdit.videoReplaced"), {
          description: t("courseEdit.videoReplacedDesc", { file: selected.split(/[\\/]/).pop() }),
        });
      } catch {
        toast.error(t("courseEdit.videoReplaceFailed"));
      }
    },
    [t],
  );

  const handleBatchVideoReplace = useCallback(
    async (sectionId: number) => {
      const selected = await open({
        multiple: true,
        title: t("courseEdit.selectVideoFiles"),
        filters: [{ name: t("courseEdit.videoFile"), extensions: videoExtensions }],
      });
      if (!selected || !Array.isArray(selected) || selected.length === 0) return;
      try {
        const newLessons = await storeBatchReplaceSectionVideos(sectionId, selected as string[]);
        setLocalSections((prev) =>
          prev.map((s) =>
            s.id !== sectionId ? s : { ...s, lessons: newLessons },
          ),
        );
        toast.success(t("courseEdit.batchVideoReplaced"), {
          description: t("courseEdit.batchVideoReplacedDesc", { count: newLessons.length }),
        });
      } catch {
        toast.error(t("courseEdit.batchVideoReplaceFailed"));
      }
    },
    [t],
  );
    ...
    (sectionId: number, lessonId: number) => {
      storeDeleteLesson(lessonId)
        .then(() => {
          ...
          toast.success(t("courseEdit.lessonDeleted"));
        })
        .catch(() => toast.error(t("courseEdit.lessonDeleteFailed")));
    },
    [t],
  );
    ...
    (sectionId: number) => {
      storeDeleteSection(sectionId)
        .then(() => {
          ...
          toast.success(t("courseEdit.sectionDeleted"));
        })
        .catch(() => toast.error(t("courseEdit.sectionDeleteFailed")));
    },
    [t],
  );
            ...
            >
              {localSections.map((section) => (
                <SortableSection
                  ...
                  onLessonVideoReplace={handleLessonVideoReplace}
                  onBatchVideoReplace={handleBatchVideoReplace}
...
function SortableSection({
  ...
  onSectionTitleChange: (sectionId: number, newTitle: string) => void;
  onLessonTitleChange: (sectionId: number, lessonId: number, newTitle: string) => void;
  onLessonVideoReplace: (sectionId: number, lessonId: number) => void;
  onBatchVideoReplace: (sectionId: number) => void;
  ...
  return (
    <div ref={setNodeRef} style={style} className="group/section">
      <div className="flex items-center gap-2 py-1.5">
        ...
        <EditableTitle
          value={section.title}
          onSave={(newTitle) => onSectionTitleChange(section.id, newTitle)}
          ariaLabel={t("courseEdit.editSectionTitle")}
          className="flex-1 font-sans text-sm font-medium text-foreground"
          inputClassName="flex-1 bg-transparent font-sans text-sm font-medium text-foreground focus:outline-none rounded px-1 py-0.5 border border-primary/30"
        />
        <button
          onClick={(e) => {
            e.stopPropagation();
            onBatchVideoReplace(section.id);
          }}
          title={t("courseEdit.batchReplaceVideos")}
          className="flex size-5 shrink-0 items-center justify-center rounded text-muted-foreground/40 opacity-0 transition-opacity hover:text-primary group-hover/section:opacity-100"
        >
        ...
        {confirmDelete ? (
          <div className="flex items-center gap-1">
            <button
              ...
              className="rounded bg-destructive/15 px-1.5 py-0.5 font-sans text-[10px] font-medium text-destructive transition-colors hover:bg-destructive/25"
            >
              {t("common.confirm")}
            </button>
            <button
              ...
              className="rounded px-1.5 py-0.5 font-sans text-[10px] font-medium text-muted-foreground transition-colors hover:text-foreground"
            >
              {t("common.cancel")}
            </button>
          </div>
          ...
          <button
            ...
            title={t("courseEdit.deleteSection")}
            className="flex size-5 shrink-0 items-center justify-center rounded text-muted-foreground/40 opacity-0 transition-opacity hover:text-destructive group-hover/section:opacity-100"
          >
            ...
            >
              {section.lessons.map((lesson, li) => (
                <SortableLesson
                  ...
                  onVideoReplace={onLessonVideoReplace}
...
function SortableLesson({
  ...
  index: number;
  sectionId: number;
  onTitleChange: (sectionId: number, lessonId: number, newTitle: string) => void;
  onVideoReplace: (sectionId: number, lessonId: number) => void;
  onDelete: (sectionId: number, lessonId: number) => void;
}) {
  const { t } = useTranslation();
  const [confirmDelete, setConfirmDelete] = useState(false);
  const {
  ...
  return (
    <div ref={setNodeRef} style={style} className="group/lesson flex items-center gap-2 py-1">
      <button
        {...attributes}
        {...listeners}
        className="cursor-grab text-muted-foreground/40 hover:text-muted-foreground touch-none"
      ...
      <EditableTitle
        value={lesson.title}
        onSave={(newTitle) => onTitleChange(sectionId, lesson.id, newTitle)}
        ariaLabel={t("courseEdit.editLessonTitle")}
        className="flex-1 font-sans text-xs text-foreground/80"
        inputClassName="flex-1 bg-transparent font-sans text-xs text-foreground focus:outline-none rounded px-1 py-0.5 border border-primary/30"
      />
      <button
        onClick={(e) => {
          e.stopPropagation();
          onVideoReplace(sectionId, lesson.id);
        }}
        title={t("courseEdit.replaceVideo")}
        className="flex size-4 shrink-0 items-center justify-center rounded text-muted-foreground/40 opacity-0 transition-opacity hover:text-primary group-hover/lesson:opacity-100"
      >
      ...
      {confirmDelete ? (
        <div className="flex items-center gap-1">
          <button
            ...
            className="rounded bg-destructive/15 px-1 py-0.5 font-sans text-[9px] font-medium text-destructive transition-colors hover:bg-destructive/25"
          >
            {t("common.confirm")}
          </button>
          <button
            ...
            className="rounded px-1 py-0.5 font-sans text-[9px] font-medium text-muted-foreground transition-colors hover:text-foreground"
          >
            {t("common.cancel")}
          </button>
        </div>
        ...
        <button
          ...
          title={t("courseEdit.deleteLesson")}
          className="flex size-4 shrink-0 items-center justify-center rounded text-muted-foreground/40 opacity-0 transition-opacity hover:text-destructive group-hover/lesson:opacity-100"
        >
  ...
  return (
    <div className="flex min-w-0 flex-1 items-center gap-1.5">
      <span
        ...
        title={t("courseEdit.doubleClickToEdit")}
      >
        {value}
      </span>
      <button
        ...
        aria-label={ariaLabel}
        className="flex size-4 shrink-0 items-center justify-center rounded text-muted-foreground/40 opacity-0 transition-opacity hover:text-foreground group-hover/section:opacity-100 group-hover/lesson:opacity-100"
      >
