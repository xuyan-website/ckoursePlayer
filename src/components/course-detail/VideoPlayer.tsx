...
export const VideoPlayer = forwardRef<VideoPlayerHandle, VideoPlayerProps>(function VideoPlayer({
  ...
  const [noteSavedTip, setNoteSavedTip] = useState(false);
  ...
  // True while waiting for the initial seek to complete — suppresses canplay/playing
  // from clearing the buffering spinner prematurely (causes double spinner)
  const waitingForInitialSeekRef = useRef(false);

  // Refs to avoid re-running the initial-seek effect when these change
  ...
  useEffect(() => {
    ...
    waitingForInitialSeekRef.current = false;
  }, [lesson?.id]);
  ...
  useEffect(() => {
    ...
    const applyInitial = () => {
      ...
      if (t == null) return; // settings not ready yet
      initialSeekAppliedRef.current = lesson.id;
      if (t > 0 && t < video.duration) {
        waitingForInitialSeekRef.current = true;
        video.currentTime = t;
      }
      video.playbackRate = playbackSpeedRef.current;
      video.volume = defaultVolumeRef.current / 100;
  ...
  const handleScreenshot = useCallback(async () => {
    const video = videoRef.current;
    if (!video || !lesson?.videoPath) return;
    try {
  ...
  const handlePlaying = useCallback(() => {
    if (waitingForInitialSeekRef.current) return;
    setIsBuffering(false);
  }, []);
    ...
    >
      ...
      {onAddNote && (
        <button
          ...
          title={t("notesPanel.writeANote")}
          className={cn(
            "absolute left-4 top-1/2 z-10 flex size-11 -translate-y-1/2 items-center justify-center rounded-full bg-black/40 text-white/90 backdrop-blur-md transition-all duration-300 hover:scale-105 hover:bg-black/60 hover:text-white",
            showControls || isSeeking ? "opacity-100" : "opacity-0 pointer-events-none",
          )}
        >
          <NotePencil className="size-5" weight="bold" />
        </button>
      )}

      <button
        onClick={handleScreenshot}
        title={t("courseDetail.screenshot")}
        className={cn(
          "absolute right-4 top-1/2 z-10 flex size-11 -translate-y-1/2 items-center justify-center rounded-full bg-black/40 text-white/90 backdrop-blur-md transition-all duration-300 hover:scale-105 hover:bg-black/60 hover:text-white",
          showControls || isSeeking ? "opacity-100" : "opacity-0 pointer-events-none",
        )}
      >
      ...
      {noteSavedTip && isFullscreen && (
        <div className="pointer-events-none absolute top-4 left-1/2 z-50 -translate-x-1/2 rounded-lg bg-green-600 px-4 py-2 font-sans text-sm font-medium text-white shadow-lg">
