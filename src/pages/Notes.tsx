...
import type { NoteWithCourse, CourseDetail } from "@/types";
import {
  getAllNotes,
  updateNote,
  deleteNote,
...
export function Notes({ className }: NotesProps) {
  ...
  const handleExportNotes = useCallback(async () => {
    if (notes.length === 0) {
      toast.error(t("notes.noNotesToExport"));
      return;
    }
    ...
    try {
      if (hasSearch) {
        const lessonGroups = new Map<string, NoteWithCourse[]>();
        for (const note of filtered) {
          const key = `${note.courseId}:${note.lessonId}`;
          if (!lessonGroups.has(key)) {
            lessonGroups.set(key, []);
          }
          lessonGroups.get(key)!.push(note);
        }

        const courseDetailCache = new Map<number, CourseDetail | null>();
        const lessons: ExportLessonData[] = [];
        for (const [, lessonNotes] of lessonGroups) {
          const courseId = lessonNotes[0].courseId;
          if (!courseDetailCache.has(courseId)) {
            courseDetailCache.set(courseId, await getCourseDetail(courseId));
          }
          const detail = courseDetailCache.get(courseId);
          const lessonSection = detail?.sections
            .find((s) => s.lessons.some((l) => l.id === lessonNotes[0].lessonId));
          const lesson = lessonSection?.lessons
            .find((l) => l.id === lessonNotes[0].lessonId);
          lessons.push({
            sectionTitle: lessonSection?.title ?? "",
            lessonTitle: lessonNotes[0].lessonTitle,
        ...
        const docs: ExportDocData[] = [{
          docTitle: `notes-${search.trim()}`,
          sections: [{ sectionTitle: "", lessons }],
        }];
        await exportNotesToZip(docs, path, "searchResult");
      } else {
        const targetNotes = courseFilter !== null
          ? notes.filter((n) => n.courseId === courseFilter)
          : notes;

        const courseGroups = new Map<number, NoteWithCourse[]>();
        for (const note of targetNotes) {
          if (!courseGroups.has(note.courseId)) {
            courseGroups.set(note.courseId, []);
          }
          courseGroups.get(note.courseId)!.push(note);
        }

        const docs: ExportDocData[] = [];
        for (const [courseId, courseNotes] of courseGroups) {
          const detail = await getCourseDetail(courseId);
          const courseTitle = courseNotes[0].courseTitle;
          const mappedLessonIds = new Set<number>();
          const sections: ExportSectionData[] = [];

          if (detail) {
            for (const section of detail.sections) {
              const sectionLessons: ExportLessonData[] = [];
              for (const lesson of section.lessons) {
                mappedLessonIds.add(lesson.id);
                const lessonNotesArr = courseNotes
              ...
              if (sectionLessons.length > 0) {
                sections.push({ sectionTitle: section.title, lessons: sectionLessons });
              }
            }
          }

          const orphanNotes = courseNotes.filter((n) => !mappedLessonIds.has(n.lessonId));
          if (orphanNotes.length > 0) {
            const orphanLessonGroups = new Map<number, NoteWithCourse[]>();
            for (const note of orphanNotes) {
              if (!orphanLessonGroups.has(note.lessonId)) {
                orphanLessonGroups.set(note.lessonId, []);
              }
              orphanLessonGroups.get(note.lessonId)!.push(note);
            }
            const orphanLessons: ExportLessonData[] = [];
            for (const [, lessonNotes] of orphanLessonGroups) {
              orphanLessons.push({
                sectionTitle: t("notes.unsortedNotes"),
                lessonTitle: lessonNotes[0].lessonTitle,
                videoPath: "",
                notes: lessonNotes
                  .sort((a, b) => a.createdAt.localeCompare(b.createdAt))
            ...
            sections.push({ sectionTitle: t("notes.unsortedNotes"), lessons: orphanLessons });
          }

          if (sections.length > 0) {
            docs.push({ docTitle: courseTitle, sections });
          }
        }
        ...
        await exportNotesToZip(docs, path, "fullCourse");
      }
