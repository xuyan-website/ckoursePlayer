...
const CURRICULUM_OPEN_KEY = "ckoursePlayer:curriculum-open";
...
function CourseDetailInner({
  ...
  const [curriculumOpen, setCurriculumOpen] = useState(() => {
    try {
      const raw = localStorage.getItem(CURRICULUM_OPEN_KEY);
  ...
  useEffect(() => {
    try {
      localStorage.setItem(CURRICULUM_OPEN_KEY, String(curriculumOpen));
    } catch { /* ignore */ }
  }, [curriculumOpen]);
  ...
  const handleToggleComplete = useCallback(
    async (lessonId: number) => {
      try {
        ...
        toast.error(t("courseDetail.couldntUpdateLesson"), {
  ...
  const handleToggleFavorite = useCallback(
    async (lessonId: number) => {
      try {
        ...
        toast.error(t("courseDetail.couldntUpdateFavorite"), {
          description: t("courseDetail.tryAgainMoment"),
        });
      }
    },
  ...
  async function handleEditNote(noteId: number, content: string, imagePaths: string[] = []) {
    try {
  ...
  async function handleDeleteNote(noteId: number) {
    try {
      ...
      toast.error(t("courseDetail.couldntDeleteNote"), {
        description: t("courseDetail.tryAgainMoment"),
      });
    }
  }
  ...
  async function handleExportNotes() {
    ...
    if (!activeSection || !activeLesson) {
      toast.error(t("courseDetail.noNotesToExport"));
      return;
    }

    const lessonNotesArr = notes
      .filter((n) => n.lessonId === activeLesson.id)
      .sort((a, b) => a.createdAt.localeCompare(b.createdAt));

    if (lessonNotesArr.length === 0) {
      toast.error(t("courseDetail.noNotesToExport"));
      return;
    }
    ...
    try {
      const docs: ExportDocData[] = [{
        docTitle: activeSection.title,
        sections: [{
      ...
      await exportNotesToZip(docs, path, "singleLesson");
  ...
  const handleOpenResource = async (path: string) => {
    try {
      ...
      toast.error(t("courseDetail.couldntOpenResource"), {
        description: t("courseDetail.fileMaybeMoved"),
      });
    }
  };
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    getLessonResources(activeLesson.id)
      .then((resources) => {
        if (!cancelled) setLessonResources(resources);
      })
      .catch((err) => {
        console.debug("getLessonResources failed", err);
        if (!cancelled) setLessonResources([]);
      });
    return () => {
      cancelled = true;
    };
  ...
  const handleAddResource = async () => {
    ...
    try {
      const selected = await openDialog({
        multiple: false,
        title: t("courseDetail.selectFile"),
      });
      if (!selected || typeof selected !== "string") return;
      await addLessonResource(course.id, activeLesson.id, selected);
      const resources = await getLessonResources(activeLesson.id);
      setLessonResources(resources);
    } catch (err) {
      console.debug("addLessonResource failed", err);
      reportError(err, "CourseDetail.handleAddResource", { severity: "expected" });
      toast.error(t("courseDetail.couldntAddResource"));
    }
  };
  ...
  const handleDeleteResource = async (resourceId: number) => {
    ...
    try {
      ...
      console.debug("deleteResource failed", err);
      reportError(err, "CourseDetail.handleDeleteResource", { severity: "expected" });
      toast.error(t("courseDetail.couldntDeleteResource"));
    }
  };

  const handleReplaceResource = async (resourceId: number) => {
    if (!activeLesson) return;
    try {
      const selected = await openDialog({
        multiple: false,
      ...
      if (!selected || typeof selected !== "string") return;
      await updateResourcePath(resourceId, selected);
      const resources = await getLessonResources(activeLesson.id);
      setLessonResources(resources);
    } catch (err) {
      console.debug("updateResourcePath failed", err);
      reportError(err, "CourseDetail.handleReplaceResource", { severity: "expected" });
      toast.error(t("courseDetail.couldntUpdateResource"));
    }
  };
  ...
  const handleRevealFile = async (path: string) => {
    try {
      ...
      console.debug("revealInExplorer failed", err);
      reportError(err, "CourseDetail.handleRevealFile", {
        path,
        severity: "expected",
      });
      toast.error(t("courseDetail.couldntOpenResource"), {
  ...
  const handleOpenFile = async (path: string) => {
    try {
      ...
      console.debug("openPath failed", err);
      reportError(err, "CourseDetail.handleOpenFile", {
        path,
        severity: "expected",
      });
      toast.error(t("courseDetail.couldntOpenResource"), {
      ...
      >
        ...
        >
          <VideoPlayer
            ...
            initialTime={settingsLoaded && activeLesson
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2 sm:gap-3">
              ...
              {course.folderPath.startsWith("gdrive:") && (
                <span
                  ...
                  title={t("courseDetail.loadedFromDrive")}
                >
                  <GoogleDriveLogo className="size-3.5 text-info" weight="fill" />
                  {t("courseDetail.googleDrive")}
                </span>
              )}
            </div>
            ...
            <div className="mt-3 flex flex-wrap items-center gap-3 sm:gap-5">
              ...
              <span className="font-mono text-xs font-medium text-muted-foreground">
                {course.completedLessons}/{course.totalLessons} {t("common.lessons")}
              </span>
              <span className="font-mono text-xs font-medium text-muted-foreground">
                {percentage}%
              </span>
          ...
          <div>
            <div className="mb-3 flex items-center gap-1">
              ...
              <button
                ...
                style={{ transitionTimingFunction: SNAPPY ,cursor:"default"}}
              >
                <span className="flex items-center gap-1.5">
            ...
            {activeTab === "resources" && (
              <div className="flex flex-col gap-3">
                  ...
                  <>
                    {lessonResources.length === 0 ? (
                      <p className="py-4 text-center font-sans text-xs text-muted-foreground/60">
                        {t("courseDetail.noOtherFiles")}
                      </p>
                    ) : (
                      lessonResources.map((resource) => {
                        const Icon = resourceIcons[resource.type] || File;
                        const fileName = resource.path.split(/[\\/]/).pop() || resource.title;
                        return (
                          <div
                            key={resource.id}
                          ...
                          >
                            <div className="flex items-center gap-2">
                              <button
                                onClick={() => setSelectedFile({ name: fileName, path: resource.path })}
                ...
                {courseData.resources.length > 0 && (
                  <div className="mt-2">
                    <div className="mb-1.5 font-sans text-xs font-medium text-muted-foreground">
                      {t("courseDetail.courseResources")}
                    </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/60 backdrop-blur-sm">
          ...
          >
            ...
            <p className="mt-2 font-sans text-xs leading-relaxed text-muted-foreground">
              {t("courseDetail.timestampFrom")}{" "}
              <span className="font-medium text-foreground">
                {pendingTimestampNav.lessonTitle}
              </span>
              . {t("courseDetail.switchingWillSave")}
            </p>
        ...
        >
          ...
          >
            ...
            <div className="mt-4 space-y-2">
              <button
                onClick={() => {
                  handleRevealFile(selectedFile.path);
                  setSelectedFile(null);
                }}
                className="flex w-full items-center gap-2 rounded-lg border border-border px-3 py-2 text-left transition-colors hover:bg-secondary"
              >
                <FolderOpen className="size-4 text-muted-foreground" />
                <span className="font-sans text-xs font-medium text-foreground">
                  {t("courseDetail.openLocation")}
                </span>
              </button>
              <button
                onClick={() => {
              ...
              >
                ...
                <span className="font-sans text-xs font-medium text-foreground">
                  {t("courseDetail.openWithDefault")}
                </span>
              </button>
            </div>
