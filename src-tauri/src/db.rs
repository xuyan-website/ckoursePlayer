use rusqlite::{params, params_from_iter, types::Value as SqlValue, types::ValueRef, Connection, Result as SqlResult};
...
pub fn init_db(app_data_dir: &Path) -> SqlResult<Connection> {
    ...
    // Notes: add image_path column for existing databases (legacy)
    let _ = conn.execute_batch(
        "ALTER TABLE notes ADD COLUMN image_path TEXT;",
    );

    // Notes: add image_paths column (JSON array) for existing databases
    let _ = conn.execute_batch(
        "ALTER TABLE notes ADD COLUMN image_paths TEXT;",
    );

    // Migrate legacy single image_path to image_paths JSON array
    {
        let migrated: Vec<(i64, String)> = conn
            .prepare("SELECT id, image_path FROM notes WHERE image_path IS NOT NULL AND image_paths IS NULL")?
            .query_map([], |row| Ok((row.get(0)?, row.get(1)?)))?
            .collect::<SqlResult<Vec<_>>>()?;
        for (id, path) in migrated {
            let json = serde_json::to_string(&vec![path]).unwrap_or_default();
            conn.execute(
                "UPDATE notes SET image_paths = ?1 WHERE id = ?2",
                params![json, id],
            )?;
        }
    }
...
pub fn add_lesson_resource(
    ...
    conn.execute(
        "INSERT INTO resources (course_id, lesson_id, title, path, resource_type) VALUES (?1, ?2, ?3, ?4, ?5)",
        params![course_id, lesson_id, title, path, resource_type],
    )?;
    Ok(conn.last_insert_rowid())
}

pub fn delete_resource(conn: &Connection, resource_id: i64) -> SqlResult<()> {
...
pub fn update_resource_path(
    ...
    conn.execute(
        "UPDATE resources SET title = ?1, path = ?2, resource_type = ?3 WHERE id = ?4",
        params![title, path, resource_type, resource_id],
    )?;
    Ok(())
}
...
pub fn get_all_notes(conn: &Connection) -> SqlResult<Vec<NoteWithCourse>> {
    let mut stmt = conn.prepare(
        "SELECT n.id, n.course_id, n.lesson_id, n.lesson_title, n.content,
                n.video_time, n.image_paths, n.created_at, n.updated_at, c.title, c.accent_color
         FROM notes n
    ...
    let notes = stmt
        .query_map([], |row| {
            let image_paths_json: Option<String> = row.get(6)?;
            let image_paths = image_paths_json
...
pub fn get_course_notes(conn: &Connection, course_id: i64) -> SqlResult<Vec<Note>> {
    let mut stmt = conn.prepare(
        "SELECT id, course_id, lesson_id, lesson_title, content, video_time, image_paths, created_at, updated_at
...
pub fn add_note(
    ...
    let now = chrono_now();
    let image_paths_json = serde_json::to_string(image_paths).unwrap_or_else(|_| "[]".to_string());
    conn.execute(
        "INSERT INTO notes (course_id, lesson_id, lesson_title, content, video_time, image_paths, created_at, updated_at)
         VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7, ?8)",
        params![course_id, lesson_id, lesson_title, content, video_time, image_paths_json, now, now],
    )?;
...
pub fn update_note(conn: &Connection, note_id: i64, content: &str, image_paths: &[String]) -> SqlResult<()> {
    let now = chrono_now();
    let image_paths_json = serde_json::to_string(image_paths).unwrap_or_else(|_| "[]".to_string());

    // Read old image_paths to clean up removed files
    let old_paths: Vec<String> = conn
        .query_row(
            "SELECT image_paths FROM notes WHERE id = ?1",
            params![note_id],
            |row| {
    ...
    for old_path in &old_paths {
        if !image_paths.contains(old_path) {
            let _ = std::fs::remove_file(old_path);
        }
    }

    conn.execute(
        "UPDATE notes SET content = ?1, image_paths = ?2, updated_at = ?3 WHERE id = ?4",
        params![content, image_paths_json, now, note_id],
    )?;
    Ok(())
}

pub fn delete_note(conn: &Connection, note_id: i64) -> SqlResult<()> {
    // Read image_paths before deleting to clean up files
...
pub fn update_section_title(conn: &Connection, section_id: i64, title: &str) -> SqlResult<()> {
    conn.execute(
...
pub fn update_lesson_title(conn: &Connection, lesson_id: i64, title: &str) -> SqlResult<()> {
    conn.execute(
        "UPDATE lessons SET title = ?1 WHERE id = ?2",
        params![title, lesson_id],
    )?;
    Ok(())
}
...
pub fn update_lesson_video_path(
    ...
    conn.execute(
        "UPDATE lessons SET video_path = ?1, last_position = 0 WHERE id = ?2",
        params![video_path, lesson_id],
    )?;
    Ok(())
}

pub fn replace_section_lessons(
    conn: &Connection,
    section_id: i64,
    new_lessons: &[(String, String, u64)],
) -> SqlResult<Vec<Lesson>> {
    conn.execute(
        "DELETE FROM lessons WHERE section_id = ?1",
        params![section_id],
    )?;

    let mut results = Vec::with_capacity(new_lessons.len());
    for (i, (title, video_path, duration_secs)) in new_lessons.iter().enumerate() {
        let duration_mins = (*duration_secs / 60) as i64;
        conn.execute(
            "INSERT INTO lessons (section_id, title, video_path, sort_order, completed, is_last_watched, duration)
             VALUES (?1, ?2, ?3, ?4, 0, 0, ?5)",
            params![section_id, title, video_path, i as i64, duration_mins],
        )?;
        let lesson_id = conn.last_insert_rowid();
        results.push(Lesson {
            id: lesson_id,
...
pub fn delete_lesson(conn: &Connection, lesson_id: i64) -> SqlResult<()> {
    conn.execute("DELETE FROM notes WHERE lesson_id = ?1", params![lesson_id])?;
    conn.execute("DELETE FROM resources WHERE lesson_id = ?1", params![lesson_id])?;
    conn.execute("DELETE FROM lessons WHERE id = ?1", params![lesson_id])?;
    Ok(())
}

pub fn delete_section(conn: &Connection, section_id: i64) -> SqlResult<()> {
    conn.execute(
        "DELETE FROM notes WHERE lesson_id IN (SELECT id FROM lessons WHERE section_id = ?1)",
        params![section_id],
    )?;
    conn.execute(
        "DELETE FROM resources WHERE lesson_id IN (SELECT id FROM lessons WHERE section_id = ?1)",
        params![section_id],
    )?;
...
fn read_table(conn: &Connection, table: &str) -> SqlResult<Vec<serde_json::Value>> {
    ...
    let rows = stmt.query_map([], |row| {
        ...
        for (i, name) in col_names.iter().enumerate() {
            let val = match row.get_ref(i)? {
                ...
                ValueRef::Text(s) => serde_json::Value::from(String::from_utf8_lossy(s).to_string()),
...
pub fn export_all_data(conn: &Connection) -> SqlResult<ExportPayload> {
    ...
    Ok(ExportPayload {
        version: 1,
        exported_at,
        courses: read_table(conn, "courses")?,
        sections: read_table(conn, "sections")?,
        lessons: read_table(conn, "lessons")?,
        ...
        settings: read_table(conn, "settings")?,
        custom_categories: read_table(conn, "custom_categories")?,
    })
}
...
fn insert_table(
    ...
    for row in rows {
        ...
        let params: Vec<SqlValue> = columns.iter().map(|c| json_to_sql_value(&obj[c])).collect();
        conn.execute(&sql, params_from_iter(params.iter()))?;
    }
    Ok(())
}

pub fn import_all_data(conn: &Connection, payload: &ExportPayload, mode: &str) -> SqlResult<()> {
    ...
    insert_table(conn, "courses", &payload.courses, replace_mode)?;
    insert_table(conn, "sections", &payload.sections, replace_mode)?;
    insert_table(conn, "lessons", &payload.lessons, replace_mode)?;
    ...
    insert_table(conn, "custom_categories", &payload.custom_categories, replace_mode)?;
    // Settings: always upsert by key.
    insert_table(conn, "settings", &payload.settings, true)?;
